...
import com.rup.feature.presentation.login.fragments.KakaoLoginRepository
...
class LoginViewModel : BaseViewModel() {
    ...
    private val _navigateToOtherScreen = MutableLiveData<Boolean>()
    val navigateToOtherScreen: LiveData<Boolean>
    ...
    fun existCheck(id: MemberExistReq) = viewModelScope.launch {
        ...
        if (exist) {
            ...
            _navigateToOtherScreen.value = true // 화면 전환 요청
        }
    }
    ...
    fun userLogin(id: MemberExistReq) = viewModelScope.launch {
        ...
        val sharedPref = RUPApplication.context.getSharedPreferences("MY_APP_PREFS", Context.MODE_PRIVATE)

        sharedPref?.edit()?.putString("ACCESS_TOKEN", accessToken)?.apply()
    }

    fun onNavigationHandled() {
        _navigateToOtherScreen.value = false
    }

//    fun userSignUp(id: SignUpReq) = viewModelScope.launch {
//        val gson = Gson()
//        val response: LoginRes =
//            gson.fromJson(retrofitInterface.userSignUp(id).toString(), LoginRes::class.java)
//        val accessToken = response.result.accessToken
//        AccountHelper.setAuthToken(accessToken.toString())
